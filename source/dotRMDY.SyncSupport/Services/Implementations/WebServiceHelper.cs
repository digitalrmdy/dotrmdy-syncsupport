...
namespace dotRMDY.SyncSupport.Services.Implementations
{
	...
	public class WebServiceHelper : IWebServiceHelper
	{
			...
			where T : class
		{
			...
			Logger.LogDebug("Executing request {CallerMethod}", callerMethod);
			...
			{
				Logger.LogDebug("Request timed out | Type: {Type} | Method: {Method}", typeof(T).Name, callerMethod);
			...
			[CallerMemberName] string? callerMethod = null)
		{
			...
			{
				Logger.LogDebug("Request timed out | Method: {Method}", callerMethod);
